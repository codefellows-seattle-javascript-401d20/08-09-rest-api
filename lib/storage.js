...
.then((exists) => {
  if(!exists) {
    ...
    fs.readJSON(process.env.STORAGE_PATH)
      .then(data => {
        console.log('Found existing data in storage: ', data);
      })
      .catch(err => {
        console.log('No valid JSON was found in storage file. Writing default JSON.');
      })
      .then(() => {
        fs.writeJSON(process.env.STORAGE_PATH, []);
      });
